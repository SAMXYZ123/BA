...
bmfs   = x(4);
b2    = exp(x(5));
alpha  = 1./(1+exp(-x(6:7))); % learning rate
lambda = 1./(1+exp(-x(8))); % eligibility parameter ([0,1]) --> short-term memory parameter; how fast fades a new information that does not correspond with prior learning within one stage and is addable to old knowledge; weights events/outcomes depending on how long ago they are in terms of influence
rep    = x(9)*eye(2); % 1 if same 1st stage choice, 0 if different one (eye is identity matrix)
