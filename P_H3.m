...
bmw_mb    = Parameter_PP(5,p);
bmw_mf    = Parameter_PP(6,p);

bmwi_mb   = Parameter_PP(7,p);
bmwi_mf   = Parameter_PP(8,p);

b2    = exp(Parameter_PP(9,p));
alpha  = 1./(1+exp(-Parameter_PP(10:11,p))); % learning rate
lambda = 1./(1+exp(-Parameter_PP(12,p))); % eligibility parameter ([0,1]) --> short-term memory parameter; how fast fades a new information that does not correspond with prior learning within one stage and is addable to old knowledge; weights events/outcomes depending on how long ago they are in terms of influence
rep    = Parameter_PP(13,p)*eye(2); % 1 if same 1st stage choice, 0 if different one (eye is identity matrix)
...
save('Choice_Prob_H3.mat', 'Prob_Model_Choice_1', 'Prob_Model_Choice_2', 'Prob_Model_Choice_all');
